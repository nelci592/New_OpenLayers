...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"kv_api.js":[function(require,module,exports) {
var latlon = [52.226317, 5.313376];
var this_map;
var antwoord;
var cluster = getUrlVars()['jaarplan']; //

haal_data_op(1); ////initialize the map

map = map('map'); //Base layer, should probably change to Open Topo at a certain stage

var OpenStreetMap_Mapnik = L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
  maxNativeZoom: 19,
  maxZoom: 24,
  attribution: '&copy; <a href="http://www.openstreetmap.org/copyright">OpenStreetMap</a>'
}).addTo(this_map); // WMS layers
...
var drawControl = new L.Control.Draw({
  ...
  draw: {
    ...
    polygon: {
      ...
      // polygons cannot intersect thenselves
      drawError: {
        color: 'red',
...
this_map.on(L.Draw.Event.CREATED, function (event) {
  var layer = event.layer; //antwoord = prompt("geef omschrijving", "omschrijving");
  ...
  $("#kenmerken_verstuur").bind('click', function (e) {
    ...
    layer.kenmerken = {
      ...
      "hersteldatum": $("#hersteldatum").val(),
      "vsmnummer": $("#vsmnummer").val(),
      "middelinzet": $("#middelinzet").val(),
      "wonummer": $("#wonummer").val(),
      "inzetnummer": $("#inzetnummer").val(),
      "hwonummer": $("#hwonummer").val(),
      "startdatum": $("#startdatum").val(),
      "starttijd": $("#starttijd").val(),
      "einddatum": $("#einddatum").val(),
      "eindtijd": $("#eindtijd").val(),
      "weeknummer": $("#weeknummer").val(),
      "type": 1
    }; // console.log( layer);

    $.colorbox.close();
  });
  drawnItems.addLayer(layer); //var json = drawnItems.toGeoJSON(); {
  // console.log(json);
...
}).addTo(this_map); //Zoom button

this_map.zoomControl.setPosition('bottomleft'); //var pulsingIcon = L.icon.pulse({iconSize:[20,20],color:'orangered'});
...
}).addTo(this_map); // Layer switch for drawnItems layer
...
toggle.addTo(this_map); // Code to combine spatial data and styling data to a string
...
function drawnItemsToJSON(ilayer) {
  ...
  for (iIndex = 0; iIndex < ditems.length; ++iIndex) {
    if (ditems[iIndex] instanceof L.Point || ditems[iIndex] instanceof L.Marker) {
      ...
      dOut1 = dOut1 + '},"geometry":{"type":"Point","coordinates":[' + ditems[iIndex]._latlng.lng + ',' + ditems[iIndex]._latlng.lat + ']},"style":{';
      dOut2 = '';
...
function haal_data_op(zoomoplaag) {
  var url_string = "https://safety.projectxplor.nl/geoserver/Safety/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=Safety:v_tasks&maxFeatures=5000&outputFormat=application/json&cql_filter=cluster=" + cluster; // make wfs request and add the layer in the drawItems
      ...
      var geojsonlayer = L.geoJson(response, {
        onEachFeature: function onEachFeature(feature, layer) {
          ...
          layer.on('click', function (e) {
            ...
            selectedFeature = e.target; //e.target.editing.enable();
          ...
          if (layer instanceof L.Point || layer instanceof L.Marker) {
            ...
            layer.bindTooltip("<b>Inzet #:</b> " + feature.properties.inzetnummer, {
              offset: this_offset,
              direction: 'top',
              opacity: 0.9,
              permanent: true,
...
},{}]},{},["../node_modules/parcel-bundler/src/builtins/hmr-runtime.js","kv_api.js"], null)
//# sourceMappingURL=/kv_api.1cec17e7.map